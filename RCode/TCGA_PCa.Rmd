...
# Introduction
In this note, we provide details on the prostate cancer enrichment analysis with different methods, including **NetGSA**, **DEGraph**, **PathNet**, **CAMERA**, **SPIA**, **Pathway-Express** and **topologyGSA**. 
...
```{r load data}
load("../Datasets/ProstateCancer.rda")
ls()
```
The variables are 
  ...
  * `ALL_EntrezID`: a list of gene Entrez IDs of length 17539. These are the genes mapped from the complete set of Affymetrix probe IDs. 
  ...
  * `dataset`: a list with values
    ...
    + `gene_info`: a data frame of gene information, including the original AffyID, gene symbol and Entrez ID. 
    + `sample_info`: a numeric vector of sample status, with 1 for cases and 0 for control. 
...
The goal is to identify KEGG pathways enriched between prostate cancer cases and normal control patients. 
...
my.iterations <- function(iters){
  ...
  dat[[1]] <- dataset$dat[,(dataset$sample_info==1)]## p by n1
  dat[[2]] <- dataset$dat[,(dataset$sample_info==0)]## p by n2
  n <- sapply(dat, ncol)
